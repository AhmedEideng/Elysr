...
`;
  ...
  const newArticle = {
    ...
    // 🛡️ Provenance صادقة: المحتوى مولّد آليًا ولا يوجد عليه مراجعة
    // بشرية/طبية — العرض (UI + JSON-LD) لازم يعكس الواقع حرفيًا.
    autoReviewed: true,
    author: {
      name: "فريق المحتوى الصحي — اليسر ميديكال",
      role: "إعداد آلي وفق معايير التحرير",
      credentials:
        "محتوى توعوي مولّد آليًا (ذكاء اصطناعي) وفق معايير الموقع: مصادر طبية موثوقة، وتحذيرات استخدام واضحة، وبدون أي وعود علاجية. لا يُعد محتوى طبيًا مُراجعًا بشريًا ولا بديلًا عن استشارة الطبيب أو الصيدلي.",
    },
    reviewer: {
      name: "الفحص الآلي للمحتوى — اليسر ميديكال",
      role: "فحص آلي للمصادر والروابط والتحذيرات",
      credentials:
        "فحص برمجي آلي (في CI): حيوية روابط المصادر، موثوقية النطاقات، وجود تحذيرات الاستخدام، ومطابقة البيانات المنظمة. هذا المحتوى لم يُراجع مراجعة طبية بشرية.",
    },
