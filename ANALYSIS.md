...
> ## ⚠️ طبيعة الوثيقة (تحديث 2026-09-16)
> هذا الملف **سجل تدقيقات/مراجعات تاريخي** (audit log) — ليس وثيقة حالة حية.
> الأقسام 1–11 توصف المشروع **كما كان وقت التحليل الأول (2026-08-26)**
> (82 منتجاً، 92 دليلًا، 246 صفحة)، وأرقامها **لم تعد دقيقة** بعد حذف
> الأدوية والمحظورين. الأقسام 12 فما بعد سجلّات مرقمة بتواريخ.
> **لا تعتمد على هذا الملف كـ source of truth — الحقيقة الكود نفسه**
> (products.ts / articles / landing-pages) والأرقام الحالية في الجدول أدناه.
>
> ### الأرقام الفعلية الحالية (2026-09-16)
>
> | العنصر | في هذا التحليل (08-26) | **الفعلي الآن** |
> |---|---|---|
> | المنتجات | 82 (52 رجال / 23 نساء / 7 أجهزة) | **78 (49 / 22 / 7)** — صفر أدوية |
> | المقالات | 56 | 56 |
> | أدلة SEO (landing) | 92 | **93** |
> | الصفحات المُسبقة | 246 | **243** (16 ثابت + 78 + 56 + 93) |
> | الـ redirects | 152 | **175** |
> | اختبارات الوحدة | 119 (10 ملفات) | **207 (16 ملفاً)** |
>
> المصدر: `npm run build` + `npm run test:unit` على الشجرة الحالية.
...
- "مبادرة الرعاية الماسية": 1000→10% / 1500→15% / 2000→20%.
...
- `articleSchema`: Article + author (د. أحمد عابد) + reviewedBy (هيئة المراجعة) + **citation من sources** + publisher.
...
4. `validateOrderPayload` (صرامة كاملة):
   ...
   - items: 1–50، qty صحيح 1–9999 (سقف sanity؛ الحد الحقيقي = فحص qty≤stock لكل منتج)، **كل منتج يجب أن يوجد في products-db.json** (مولّد من نفس الكود وقت البناء)، stock>0، qty≤stock.
...
- من غير باقة → يعمل الخصم المتدرج (10/15/20%) كالمعتاد.
...
- author: "د. أحمد عابد — فريق المحتوى الصحي" (بكالوريوس صيدلة)
...
## 35) Provenance المقالات — تصحيح "مراجعة بشرية" غير موجودة (2026-09-05)

### القرار (من المالك)
المقالات **لا يقرأها/يراجعها إنسان** — النشر أوتوماتيك. يعني كل عرض
"مراجعة بشرية/لجنة" على الموقع كان **ادعاء غير حقيقي**.

### الواقع اللي اتوثق ضد الكود
- الـ 56 مقالة الحالية: author "د. أحمد عابد (صيدلي)" + reviewer
  "المراجعة الداخلية" (ثوابت مشتركة) — محتوى المالك نفسه (اتترك
  زي ما هو: مسؤوليته التحريرية المباشرة).
- الـ generator (لما ينشر) كان حاطط reviewer "قسم مراجعة المحتوى —
  اليسر ميديكال" = انطباع فريق مراجعة بشري لمحتوى AI — **ده الباج**.
- JSON-LD المقال كان بيصدر `reviewedBy` (Organization) لكل مقال.
- صفحة /medical-review-board كانت بتزعم: "مراجعة الدقة/السلامة"
  كمراحل بشرية + "مراجعة دورية كل 3-6 أشهر" + "فريق المراجعة
  الداخلية" — كلها غير حقيقية.
- عندها كمان: عن عن pre-render عن /medical-review-board و/about
  (اللي يشوفه non-JS crawlers) كان فيه نفس الادعاءات.

### اللي اتطبق (كله متحقق: lint 0, tsc 0, integrity 0, schemas 0,
172 وحدة, 18 e2e)
1. **generator**: مقالات الـ AI الجديدة دلوقتي:
   `autoReviewed: true` + author "فريق المحتوى الصحي" (تصريح صريح:
   "مولّد آليًا... لا يُعد محتوى طبيًا مُراجعًا بشريًا") + reviewer
   "الفحص الآلي للمحتوى" (يصف الفحص البرمجي اللي فعلاً شغّال في CI).
2. **JSON-LD**: `reviewedBy` **ما يتصدرش** للمقالات
   `autoReviewed` (مفيش مزاعم مراجعة في structured data) — مقالات
   المالك الأصلي محتفظا بيه (قرار تحريري له).
3. **صفحة الشفافية** (/medical-review-board) اتكتبت من جديد حسب
   الواقع: من يكتب (د. أحمد عابد شخصيًا) + الفحص الآلي (CI) + **قسم
   صريح "بما في ذلك ما لا نفعله"** (مفيش لجنة دائمة، مفيش فريق
   مراجعة مؤسسي، مفيش إعادة مراجعة دورية) + مصادر + حدود
   المسؤولية. الـ title/description اتعدّلوا ("شفافية المحتوى").
4. **pre-render**: محتوى /medical-review-board و/about (للـ
   non-JS crawlers) اتعدل بنفس الصياغة الصادقة (اتتحقق من dist).
5. **about.tsx**: "فريق المحتوى والمراجعة" → "فريق المحتوى"
   (صياغة صادقة) + كارت اللينك "شفافية المحتوى".
6. **landing page**: "بمراجعة داخلية للسلامة والتحذيرات" →
   "بمصادر موثوقة وتحذيرات واضحة وفحص آلي للروابط".

### ملاحظة تصميم
الفلسفة: **الموقع يقول الحقيقة على 3 طبقات** (UI + JSON-LD +
pre-render) بنفس الصياغة — وأي محتوى مستقبلي AI لازم يمر بنفس
الوسم `autoReviewed` عشان العرض + الـ schema يبقى صادق تلقائيًا.

## 36) توحيد صياغة المقالات (قديمة + جديدة) — صدق من غير قلق على القارئ (2026-09-05)

### القرار (من المالك)
صياغة **موحدة** لكل المقالات — القديمة والجديدة — تكون صادقة
ومطمئنة في نفس الوقت (مش "مراجع آليًا" لكل حاجة، ولا "مراجع
بشريًا" لكل حاجة، ولا نصوص بتخوّف القارئ زي "لم يُراجع بشريًا").

### الصياغة الموحدة (ثوابت مشتركة واحدة لكل الـ 56 + أي جديد)
- **author**: "فريق المحتوى الصحي — اليسر ميديكال" /
  "إعداد المحتوى تحت المسؤولية التحريرية" / "يُنشر تحت
  المسؤولية التحريرية المباشرة لد. أحمد عابد (صيدلي — مؤسس)،
  بمصادر موثوقة (WHO, Mayo Clinic, Cleveland Clinic, NIH)
  وتحذيرات واضحة. توعوي، بلا وعود علاجية، ومش بديل عن
  الاستشارة."
- **reviewer**: "فحص سلامة المحتوى — اليسر ميديكال" /
  "فحص آلي للمصادر والروابط والتحذيرات" / يصف الـ CI check
  اللي شغّال فعليًا على كل المقالات (روابط حية، نطاقات
  موثوقة، تحذيرات موجودة، بيانات منظمة).

### التعديلات
1. `articles.ts`: الثوابت اتحدت + **الـ 5 بلوكات الصريحة**
   (مقالات خاصة كانت بتكرار النص inline) اتستبدلت بـ
   `author: articleAuthor, reviewer: articleReviewer` — دلوقتي
   مفيش تاني صياغة ثانية في أي مقال.
2. `auto-generate-article.mjs`: نفس الصياغة حرفيًا (العرض
   متطابق 100% بين قديم وجديد) + `autoReviewed: true` باقى
   **للـ JSON-LD فقط** (reviewedBy ما يتصدرش للمقالات الآلية) —
   القارئ متعرضش لنص مخيف، والـ structured data هو الأهم
   عند المحركات.
3. صفحة الشفافية + pre-render: وعد "الـ AI هيتعلن في المقالة
   نفسها" اتبدل بـ disclosure على مستوى السياسة: "المحتوى
   الجديد — بما في ذلك ما يُنجز بمساعدة AI — تحت نفس
   المسؤولية التحريرية ونفس الفحص الآلي".
4. JSON-LD للمقالات القديمة (محتوى المالك): reviewedBy بقى
   "فحص سلامة المحتوى" — الفحص ده شغّال فعليًا على مقالاته
   كمان (صادق).
...
## 39) إلغاء أي حظر على m-38/m-43/m-45 — قرار المالك (2026-09-06)

### القرار
بعد مراجعة الأدلة (MC: "مطابق" في الزحف لـ Power 36/Procomil Fort،
"غير مطابق" لـ فياجرا الحريم اللي اتحذفت)، المالك قرر **إلغاء أي حظر**
على التلاتة الباقين: يدخلوا الخلاصة والسيتماب ويُفهرسوا عادي.

### ⚠️ تنبيه مخاطر موثق (قبل التنفيذ)
- "مطابق" في الزحف = الفلتر الأوتوماتيك فوّت الأسماء، **مش اعتماد**.
  مراجعة الـ feed أعمق (المكونات فيها "Sildenafil Citrate" حرفيًا +
  اسم m-45 "Viagra") — احتمال الرفض عالي.
...
- server/index.js: NOINDEX lists فاضية
- tests: compliance (القايمة فاضية + size 0) + data-integrity (مفيش
  قواعد noindex للتلاتة) + e2e (power-36 منتج عادي indexable)
- الـ feed بقى 82 (79 + 3) والسيتماب 82 منتجات — تلقائي من القايمة
...
## 42) قرار المالك: الاحتفاظ بنظام التقييمات المولّدة (2026-09-08)

### خلفية (موجزة — التفاصيل في تقرير المراجعة الشاملة 2026-09-08)
نظام التقييمات الحالي 4 طبقات: rating/reviews مخلقة في `products/*.ts` +
جينرатора حتمي في `product-reviews.ts` + قسم `ProductReviews` معنون
"تجارب حقيقية وموثقة" + `aggregateRating` من نفس الأرقام في JSON-LD.

### المخاطر المُبلَّغ عنها للمالك (مُسجلة للشفافية)
1. **سياسة جوجل**: تقييمات مصطنعة = structured data spam — إجراء يدوي
   ممكن يكون على مستوى الموقع كله (خاصة مع تاريخ الرفض الدوائي السابق).
2. **قانوني (مصر)**: "تجارب حقيقية وموثقة" لمحتوى مولّد = إعلان مضلل
   محتمل (حماية المستهلك/المنافسة) — خاصة مع وجود قسم المراجعات
   الحقيقية (CustomerReviews) تحته مباشرة.
3. **تناقض داخلي** مع مسار "المحتوى الصادق" (اسم الدكتور، provenance
   المقالات، صفحة المراجعة).

### القرار
**المالك اختار الاحتفاظ بالنظام كما هو (2026-09-08)** بعد اطلاع كامل
على المخاطر — قرار تجاري/قانوني وعيه. مفيش أي تغيير كود.

### مخرج مستقبلي لو تغيّر القرار
- شال قسم `ProductReviews` من `products.$slug.tsx` (سطر 577)
...
## 44) خط أساس المخزون 5000 لكل منتج + رفع سقف qty (2026-09-10)

### القرار
المالك أكد أن المخزون الفعلي **بالآلاف** لكل المنتجات. القيم القديمة
(50–200) كانت أقل من الواقع — فأُطبّق:

1. **`stock: 5000`** لكل الـ 78 منتجاً (`men.ts` 49 · `women.ts` 22 ·
   `devices.ts` 7) — `api/lib/products-db.json` اتولّد من نفس المصادر
   (drift guard شغّال).
2. **رفع سقف qty في `api/submit-order.js` من 999 → 9999**: السقف القديم
   كان أقل من خط الأساس الجديد ففحص "Quantity exceeds stock" بقى
   غير قابل للبلوغ (وكان فشل الاختبار اللي بيرجع `qty = stock + 1`).
   السقف الجديد sanity cap بس — الحد الحقيقي هو فحص المخزون لكل منتج.

### الأثر
- شارة النُدرة الوهمية "باقي X فقط" (كانت بتتعرض عند stock ≤ 5) **ما
  بتظهرش تاني** — سلوك أصدق مع الواقع.
- Google Shopping feed: 78/78 منتج `in stock`.
- JSON-LD: `InStock` لكل المنتجات.
- الـ stepper في صفحة المنتج مبني على `product.stock` (5000) — تلقائي.

### التحقق
- `npm run ci` كاملة خضرا: lint + tsc + data-integrity + 177 وحدة +
