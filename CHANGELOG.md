...
- **Unused `.avif` purge (2026-09-14)**: the 78 `.avif` copies of every
  live product image (generated by the manual `optimize-images.mjs` run)
  were referenced nowhere — the site serves `.webp` only. Pre-deletion
  verification: zero strict references in code/DB/feeds/dist, every `.avif`
  had a referenced `.webp` twin, and post-deletion all 134 unique image
