...
const dailyTips = [
  ...
  {
    headline: "🔥 الأكثر طلباً هذا الأسبوع — اختيارات حقيقية من عملاء حقيقيين",
    text: "هذا الأسبوع، اختار مئات العملاء منتجات محددة كأفضل قيمة: كبسولات Hammer of Thor (4.9⭐ من 312 عميل) للكفاءة العالية، عسل Top Sellers الملكي (200 ج.م فقط!) للميزانية، وقطرات Lady Era للنساء (520 تقييم إيجابي). هذه ليست مجرد أرقام — بل ثقة حقيقية من أشخاص حقيقيين.",
...
export function DailyAdvice() {
  // اختيار النصيحة بناءً على اليوم (تتغير يومياً بشكل تلقائي وثابت لكل الزوار)
  const getDailyTip = () => {
    const date = new Date();
    // حساب رقم اليوم في السنة (يتغير كل يوم)
