...
export function FloatingActions() {
  ...
  const isProductDetailsPage = /^\/products\/(?!men$|women$|devices$|guides\/).+/.test(pathname);
  const hideFloatingWhatsApp = isProductDetailsPage || pathname === "/cart";

  useEffect(() => {
    if (hideFloatingWhatsApp) {
  ...
  // صفحات تفاصيل المنتجات والسلة لديها أزرار واتساب واضحة؛ إخفاء الزر العائم يمنع التزاحم.
  if (hideFloatingWhatsApp) return null;
    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
      {showNudge && (
        <div className="relative max-w-[260px] rounded-2xl bg-white border border-border shadow-2xl p-3 pr-3 pl-9 animate-in slide-in-from-bottom-4 duration-300">
          <button
            onClick={dismiss}
            className="absolute top-2 left-2 inline-flex h-5 w-5 items-center justify-center rounded-full text-muted-foreground hover:bg-accent"
            aria-label="إغلاق"
          >
            <X className="h-3 w-3" />
          </button>
          <p className="text-[10px] font-bold text-foreground leading-relaxed text-center">
            👋 مستعدون لمساعدتك
            <br />
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="relative inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#25D366] text-white shadow-2xl transition-smooth hover:scale-110"
