...
export const Route = createFileRoute("/about")({
  head: () => ({
    meta: [
      { title: "من نحن — اليسر ميديكال | أفضل شركة متخصصة" },
      {
        name: "description",
        content:
          "تعرف على اليسر ميديكال، أفضل شركة متخصصة في منتجات الصحة الزوجية الأصلية في مصر، مع كتالوج مختار بعناية وشحن سري ودفع عند الاستلام.",
      },
    ],
  }),
  component: AboutPage,
...
function AboutPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-10 md:py-14 max-w-5xl">
      <PageHero
        eyebrow="من نحن"
        title="اليسر ميديكال — أفضل شركة متخصصة"
        description="اليسر ميديكال أفضل شركة متخصصة في توفير منتجات الصحة الزوجية الأصلية للرجال والنساء في مصر، بكتالوج مختار بعناية، وصف واضح، شحن سري، ودفع عند الاستلام."
      />
      ...
      <section className="mt-10 rounded-[2rem] border bg-card p-6 shadow-card md:p-8">
        ...
        <div className="space-y-4 text-sm leading-8 text-muted-foreground">
          ...
          <p>
            نعمل على مراجعة مصادر التوريد ووصف المنتجات، ونوفر نظام شحن سري يحترم خصوصية كل عميل،
            ومنصة إلكترونية تجعل تجربة الشراء أبسط وأكثر وضوحاً.
          </p>
          <p>
            اليوم نحدّث كتالوجاً يضم{" "}
            <strong className="text-foreground">{products.length} منتجاً</strong>، مع توصيل سري إلى{" "}
            <strong className="text-foreground">27 محافظة</strong> والدفع عند الاستلام حيثما كان
            متاحاً.
          </p>
        </div>
      </section>
      ...
      <section className="mt-8 grid gap-4 grid-cols-2 md:grid-cols-4">
        {[
          { n: String(products.length), label: "منتج في الكتالوج", icon: Package },
          { n: "27", label: "محافظة شحن", icon: MapPin },
          { n: "14", label: "يوم في سياسة الإرجاع", icon: Clock },
        ].map((s) => (
      ...
      <section className="mt-8 rounded-[2rem] border bg-card p-6 shadow-card md:p-8">
        <h2 className="text-2xl font-black text-center mb-6">لماذا يختار بعض العملاء اليسر؟</h2>
        <div className="grid gap-4 md:grid-cols-2">
          {[
            {
              icon: CheckCircle,
              t: "منتجات من مصادر موثوقة",
            ...
            },
            {
              icon: Lock,
              t: "خصوصية مطلقة",
              d: "تغليف محايد لا يدل على المحتوى. نوضح سياسة الخصوصية، ولا نعرض بيانات الطلب إلا للجهات اللازمة لتجهيزه وتوصيله.",
            },
            {
              icon: Truck,
              t: "توصيل لكل مصر",
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ...
            },
            {
              icon: BookOpenCheck,
              title: "فريق المحتوى",
              desc: "د. أحمد عابد (صيدلي) يكتب المحتوى التعليمي ووصف المنتجات بمسؤوليته التحريرية المباشرة، ويتحقق الفحص الآلي (CI) من المصادر والتحذيرات. نلتزم بعدم تقديم أي وعود علاجية.",
            },
            {
              icon: Headphones,
          ...
          {[
            {
              icon: Award,
              t: "الخبرة (Experience)",
              d: "نستخدم مصادر صحية منشورة في المقالات التوعوية، ونوضح حدود المحتوى وننصح بالرجوع إلى الطبيب أو الصيدلي عند الحاجة.",
            },
            {
              icon: BookOpenCheck,
              t: "التخصص (Expertise)",
              d: `${ARTICLE_COUNT} مقالة توعوية بمصادر مذكورة من WHO وMayo Clinic وCleveland Clinic، مع نظام تصنيف داخلي للمنتجات والتحذيرات.`,
            },
            {
              icon: Globe,
              t: "السلطة (Authoritativeness)",
              d: "نوضح مصدر المعلومات وسياسات الشحن والاسترجاع، ونفصل بين المراجعات المعتمدة من API والمحتوى التسويقي.",
            },
            {
              icon: ShieldCheck,
              t: "الثقة (Trustworthiness)",
              d: "شحن سري وتغليف محايد، ودفع عند الاستلام حيثما كان متاحاً، وسياسة استرجاع منشورة لمدة 14 يوماً. لا وعود علاجية.",
            },
          ].map((x) => (
