...
function CartPage() {
  ...
  const checkout = async () => {
    ...
    if (!sheetResult.success) {
      console.warn("إشعار Google Sheets: الطلب محفوظ محلياً كاحتياط");
    }
    ...
    <div className="container mx-auto px-3 sm:px-4 py-6 md:py-10">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-3">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="h-14 w-14 sm:h-16 sm:w-16 shrink-0 overflow-hidden rounded-xl border bg-muted">
                      {it.image ? (
                        <img src={it.image} alt={it.name} className="h-full w-full object-cover" width={64} height={64} loading="lazy" />
                      ) : (
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-4 sm:gap-3 shrink-0">
                    <div className="flex items-center rounded-full border bg-background">
                      <button onClick={() => setQty(it.id, it.qty - 1)} className="p-1.5 hover:bg-accent rounded-r-full transition-smooth" aria-label="تقليل الكمية">
                      ...
                      <button onClick={() => setQty(it.id, it.qty + 1)} disabled={atLimit}
                        className={`p-1.5 rounded-l-full transition-smooth ${atLimit ? "opacity-30 cursor-not-allowed" : "hover:bg-accent"}`}
                        aria-label="زيادة الكمية">
                        <Plus className="h-3.5 w-3.5 sm:h-3 sm:w-3" />
                      </button>
                    </div>
                    <button onClick={() => remove(it.id)} className="text-destructive p-2 hover:bg-destructive/10 rounded-full transition-smooth" aria-label="حذف المنتج">
          ...
          <button onClick={clear} className="text-xs text-muted-foreground hover:text-destructive px-2 transition-smooth">
            تفريغ السلة
          </button>
        </div>
        ...
        <aside className="rounded-3xl border bg-card p-5 sm:p-6 h-fit shadow-card space-y-5 lg:sticky lg:top-24 border-primary/10">
          ...
          <div className="grid grid-cols-2 gap-2 p-1.5 bg-muted rounded-2xl">
            <button onClick={() => setMethod("whatsapp")}
              className={`flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all duration-300 ${
                method === "whatsapp" ? "bg-background shadow-md text-[#25D366] scale-105" : "text-muted-foreground hover:bg-background/50"
              }`}>
              <MessageCircle className="h-4 w-4" /> واتساب
            </button>
            <button onClick={() => setMethod("direct")}
              className={`flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all duration-300 ${
                method === "direct" ? "bg-background shadow-md text-primary scale-105" : "text-muted-foreground hover:bg-background/50"
              }`}>
          ...
          <div className="rounded-xl bg-muted/50 p-3 text-[11px] text-muted-foreground leading-relaxed">
            {method === "whatsapp" ? (
              <span>📱 سيُفتح واتساب برسالة جاهزة تحتوي على تفاصيل طلبك. أرسل الرسالة ثم عد هنا لتأكيد الطلب.</span>
            ) : (
              <span>📊 سيُرسل طلبك مباشرة إلى فريق المبيعات. سنتواصل معك عبر الهاتف لتأكيد التفاصيل.</span>
            )}
          </div>
          ...
          <div className="space-y-3.5">
            ...
            <label className="block">
              ...
              <div className="relative">
                <select value={customer.governorate} onChange={(e) => setCustomer({ ...customer, governorate: e.target.value })}
                  className="w-full rounded-xl border bg-background px-3 py-2.5 text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all appearance-none cursor-pointer" required>
            ...
            <label className="block">
              ...
              <textarea value={customer.notes} placeholder="أي تفاصيل أخرى..." onChange={(e) => setCustomer({ ...customer, notes: e.target.value })}
                maxLength={300} rows={2} className="w-full rounded-xl border bg-background px-3 py-2 text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all resize-none" />
            </label>
          </div>
          ...
          <button onClick={checkout} disabled={submitting}
            className={`w-full rounded-full px-5 sm:px-6 py-3.5 sm:py-4 font-bold text-white shadow-elegant transition-all duration-300 hover:scale-[1.02] active:scale-95 disabled:opacity-60 disabled:cursor-wait flex items-center justify-center gap-2 text-sm sm:text-base ${
              method === "whatsapp" ? "bg-[#25D366] hover:bg-[#1ebd57] shadow-[#25d366]/20" : "bg-gradient-brand shadow-primary/20"
            }`}>
            {submitting ? (
